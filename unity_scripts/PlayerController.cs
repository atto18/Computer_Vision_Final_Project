...
public class PlayerController : MonoBehaviour
{
    ...
    public bool autoRetreatEnabled = true;
    public float autoRetreatDistance = 12f;
    public float autoRetreatSpeedMultiplier = 1.0f;
    public bool autoCircleArena = true;
    public float preferredEnemyDistance = 10f;
    public float autoCircleSpeed = 0.8f;
    public bool autoLookAtEnemyEnabled = true;
    public float autoLookSpeed = 6f;
    ...
    void HandleMovement()
    {
        ...
        // The player moves by himself around the arena.
        // If enemies get too close, he retreats while circling, so you can focus on gestures.
        if (autoRetreatEnabled)
        {
            Transform closestEnemy = FindClosestEnemyForMovement();

            if (closestEnemy != null)
            {
                Vector3 fromEnemy = transform.position - closestEnemy.position;
                fromEnemy.y = 0f;

                float closestDistance = fromEnemy.magnitude;

                if (closestDistance > 0.01f)
                {
                    Vector3 awayFromEnemy = fromEnemy.normalized;
                    Vector3 towardEnemy = -awayFromEnemy;

                    // Sideways movement creates circular movement around the enemy/arena.
                    Vector3 sideways = Vector3.Cross(Vector3.up, awayFromEnemy).normalized;

                    if (closestDistance < autoRetreatDistance)
                    {
                        // Enemy is close: move backward + sideways.
                        move += awayFromEnemy * autoRetreatSpeedMultiplier;
                        move += sideways * autoCircleSpeed;
                    }
                    else if (autoCircleArena)
                    {
                        // Enemy is not too close: keep moving around instead of standing still.
                        move += sideways * autoCircleSpeed;

                        // Keep a comfortable distance: not too far, not too close.
                        if (closestDistance > preferredEnemyDistance + 3f)
                        {
                            move += towardEnemy * 0.35f;
                        }
                        else if (closestDistance < preferredEnemyDistance - 2f)
                        {
                            move += awayFromEnemy * 0.35f;
                        }
                    }
                }
            }
    ...
    Transform FindClosestEnemyForMovement()
    {
        Transform closestEnemy = null;
        float closestDistance = Mathf.Infinity;

        // First method: enemies tagged as "Enemy"
        ...
        // Backup method: enemies that have EnemyAI, even if tag was forgotten
        EnemyAI[] enemiesByScript = FindObjectsOfType<EnemyAI>();
        foreach (EnemyAI enemyAI in enemiesByScript)
        {
            if (enemyAI == null || enemyAI.IsDead) continue;

            float dist = Vector3.Distance(transform.position, enemyAI.transform.position);
            if (dist < closestDistance)
            {
                closestDistance = dist;
                closestEnemy = enemyAI.transform;
            }
        }
    ...
    Transform FindClosestEnemyForAutoLook()
    {
    ...
    void AutoLookAtClosestEnemy()
    {
        if (!autoLookAtEnemyEnabled) return;

        Transform closestEnemy = FindClosestEnemyForAutoLook();
        if (closestEnemy == null) return;

        Vector3 lookDir = closestEnemy.position - transform.position;
        lookDir.y = 0f;
